  ...
  var loadMyCartEvent = function (targetSelector) {
    ...
    if (!$("#" + idCartModal).length) {
      $('body').append(
        ...
        '<h4 class="modal-title" id="myModalLabel"><span class="glyphicon glyphicon-shopping-cart"></span> Meu Carrinho </h4>' +
        '</div>' +
        ...
        '<button type="button" class="btn btn-success ' + classCheckoutCart + '">Realizar pagamento</button>' +
        '</div>' +
        '</div>' +
        '</div>' +
        '</div>'
    ...
    var drawTable = function () {
      ...
      $.each(products, function () {
        ...
        $cartTable.append(
          ...
          '<td>' + this.name + '</td>' +
          '<td title="Preço do produto" class="text-right">' + options.currencySymbol + MathHelper.getRoundedNumber(this.price) + '</td>' +
